...
def callback(ch, method, properties, body):
    ...
    print(f"[TEMPERATURA] Received message: {message}")

    # Guarda el mensaje en Redis y mide el tiempo de ejecución
    start_time = time.time()
    result = redis_client.rpush('data_temp', message)
    end_time = time.time()
...
channel.queue_bind(exchange='topic_logs', queue=queue_name, routing_key='temperatura')

channel.basic_consume(queue=queue_name, on_message_callback=callback, auto_ack=True)

print("Starting Consuming [TEMPERATURA]")
