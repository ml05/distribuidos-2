...
def callback(ch, method, properties, body):
    ...
    print(f"[LUMINOSIDAD] Received message: {message}")

    # Guarda el mensaje en Redis y mide el tiempo de ejecución
    start_time = time.time()
    result = redis_client.rpush('data_luminosidad', message)
    end_time = time.time()
    ...
    timestamp = time.time()
    with open('data_luminosidad.csv', 'a', newline='') as csvfile:
...
channel.queue_bind(exchange='topic_logs', queue=queue_name, routing_key='luminosidad')

channel.basic_consume(queue=queue_name, on_message_callback=callback, auto_ack=True)

print("Starting Consuming [LUMINOSIDAD]")

# Borra el archivo CSV existente antes de comenzar
if os.path.exists('data_luminosidad.csv'):
    os.remove('data_luminosidad.csv')
