...
def main():
    ...
    # Enlaza la cola a la clave de enrutamiento "luminosidad"
    channel.queue_bind(exchange='sensor_data', queue=queue_name, routing_key='luminosidad')

    def callback(ch, method, properties, body):
        sender = properties.headers.get('sender')
        message = body.decode('utf-8')
        print("Device", sender, "sending:", message)
        guardar_en_csv('luminosidad', message)
